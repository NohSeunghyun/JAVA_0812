...
class Buyer {
	...
	void summary() {
		...
		int cntTv = 0, cntCom = 0, cntAud = 0; // ★각 제품을 몇대 샀는지 카운팅변수
		
		// 반복문을 이용해 구입한 제품의 총 가격과 목록을 만들어 출력
		for(i = 0 ; i < item.length ; i++) { // i는 Index 역할도 할 수 있고 구입한 제품의 총 개수의 역할도 할 수 있다.
			...
			switch (item[i].toString().toUpperCase()) { // toUpperCase()로 대문자로 변환하여 케이스로 들어갈 수 있도록 한다.
				case "TV" : {
					cntTv++;
					break;
				}
